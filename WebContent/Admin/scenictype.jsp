...
<body>
	<table id="scenictype_dg" class="easyui-datagrid" style="height: 670px;"
		url="Admin/Scenictype?action=findAllScenictype" toolbar="#scenictype_toolbar" pagination="true"
	...
	<script type="text/javascript">
		...
		function newScenictype() {
			...
			url = 'Admin/Scenictype?action=addScenictype';
		...
		function editScenictype() {
			...
			if (row) {
				...
				url = 'Admin/Scenictype?action=updateScenictype&stid=' + row.stid;
			}
		...
		function destroyScenictype() {
			...
			if (row) {
				$.messager.confirm('Confirm', '确定要删除吗?', function(r) {
					if (r) {
						$.post('Admin/Scenictype?action=deleteScenictype', {
							stid : row.stid
						}, function(result) {
							if (result.success) {
								$('#scenictype_dg').datagrid('reload'); // reload the scenictype data
